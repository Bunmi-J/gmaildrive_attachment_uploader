...
def download_email_attachments():
    ...
    """
    try:
        if not all([EMAIL_USER, EMAIL_PASS, TRUSTED_SENDER, DRIVE_FOLDER_ID]):
        ...
        status, messages = mail.search(None, f'(UNSEEN FROM "{TRUSTED_SENDER}")')
        if status != 'OK':
        ...
        if not msg_ids:
            logging.info(f"No new emails from {TRUSTED_SENDER}.")
            return
